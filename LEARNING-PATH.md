...
2. **Работающий пользователь:** Разделы 4-7 (инструменты, ИИ-агенты и роли, Pack, качество). По мере необходимости.
3. **Продвинутый:** Разделы 8-12 (процессы, онтология, SOTA, бот, стратегия).
4. **Справочник:** Раздел 13 — быстрый поиск ответов.

> **Терминология:** IWE = интеллектуальная рабочая среда (то, что вы используете). Экзокортекс = инфраструктурный стек (Git, MCP, агенты), на котором построена IWE.
...
21 пара понятий, которые **нельзя путать**. Путаница в этих парах — главный источник ошибок.
...
| 5 | Роль ≠ Агент ≠ Инструмент | Маска vs. кто/что носит маску vs. инструмент (DP.D.033) |
| 6 | Метод ≠ Навык | Воспроизводимый процесс vs. личная способность |
| 7 | Наблюдение ≠ Суждение | Факт vs. интерпретация факта |
| 8 | Данные ≠ Инсайт | Записанное vs. заключённое |
| 9 | Артефакт ≠ Процесс | Продукт (существительное) vs. деятельность (глагол) |
| 10 | Pack-контент ≠ Governance-контент | Что истинно в домене vs. что делаем в проекте |
| 11 | Процесс ≠ Сервис ≠ Сценарий | Логика внутри системы vs. точка входа vs. межсистемный путь |
| 12-15 | Описание ≠ Знание, Мировоззрение ≠ Описание, Стратегический DDD ≠ Тактический, Model ≠ View | Онтологические различения из Pack DP |
| 16-18 | DSL ≠ DSLM, Capture During ≠ After, 4 уровня принципов | Методологические различения |
| 19-21 | Runbook ≠ Troubleshooting, PD ≠ EDU ≠ MIM, Anti-Bystander | Операционные различения |

**Где изучить:**
- [memory/hard-distinctions.md](./memory/hard-distinctions.md) — все 21 пара с примерами и тестами
...
| **Downstream** | 3-й | Производные от Pack | Нет | DS-ai-systems, DS-my-strategy |

**Ключевое:** Pack — **единственный** source-of-truth для доменного знания. Downstream потребляет, не создаёт.

**Подтипы Downstream:**
- `instrument` — код, боты, агенты (DS-aist-bot, DS-ai-systems)
...
| `DS-` | Downstream | DS-ai-systems, DS-my-strategy |
...
| `hard-distinctions.md` | 21 жёсткое различение | При путанице в терминах |
...
В IWE роль описывается **независимо от исполнителя**. Сначала определяем: что нужно делать, какие обязательства, какие рабочие продукты. Потом выбираем: кто это исполняет (bash-скрипт, Claude, человек). Человек и ИИ работают как команда, распределяя роли по компетенциям: ИИ берёт на себя рутинную формализацию и мониторинг, человек — стратегические решения и творческую работу.
...
**Где изучить:**
- Каталог платформенных ролей (21 роль, R1-R21): [DP.AGENT.001 §3.2](https://github.com/TserenTserenov/PACK-digital-platform/blob/main/pack/digital-platform/02-domain-entities/DP.AGENT.001-ai-agents.md) в PACK-digital-platform
- Cross-Pack Role Index (35 ролей: 21 DP + 7 EDU + 4 PD + 3 MIM): [DP.AGENT.001 §3.3](https://github.com/TserenTserenov/PACK-digital-platform/blob/main/pack/digital-platform/02-domain-entities/DP.AGENT.001-ai-agents.md)
- Архитектурное обоснование: [DP.D.033 (Role-Centric Architecture)](https://github.com/TserenTserenov/PACK-digital-platform/blob/main/pack/digital-platform/01-domain-contract/DP.D.033-role-centric-architecture.md)
...
**Где изучить:**
- [DS-ai-systems/strategist/README.md](https://github.com/TserenTserenov/DS-ai-systems/tree/main/strategist) — полная документация
- [DS-ai-systems/strategist/prompts/](https://github.com/TserenTserenov/DS-ai-systems/tree/main/strategist/prompts) — 10 промптов для каждого сценария

### 5.2. Экстрактор знаний (R2)
...
**Где изучить:**
- [DS-ai-systems/extractor/CLAUDE.md](https://github.com/TserenTserenov/DS-ai-systems/tree/main/extractor) — инструкции и пайплайн
- [DS-ai-systems/extractor/prompts/](https://github.com/TserenTserenov/DS-ai-systems/tree/main/extractor/prompts) — 7 промптов
...
**Где изучить:**
- [DS-ai-systems/synchronizer/README.md](https://github.com/TserenTserenov/DS-ai-systems/tree/main/synchronizer) — документация
- [DS-ai-systems/synchronizer/CLAUDE.md](https://github.com/TserenTserenov/DS-ai-systems/blob/main/synchronizer/CLAUDE.md) — инструкции
...
**Где изучить:**
- [DS-ai-systems/fixer/README.md](https://github.com/TserenTserenov/DS-ai-systems/tree/main/fixer) — уровни и безопасность
- [DS-ai-systems/fixer/CLAUDE.md](https://github.com/TserenTserenov/DS-ai-systems/blob/main/fixer/CLAUDE.md) — инструкции
...
**Где изучить:**
- [DS-ai-systems/pulse/README.md](https://github.com/TserenTserenov/DS-ai-systems/tree/main/pulse) — метрики, тиры, режимы
- [DS-ai-systems/pulse/CLAUDE.md](https://github.com/TserenTserenov/DS-ai-systems/blob/main/pulse/CLAUDE.md) — инструкции
...
R8 Синхронизатор (диспетчер)
    ...
    ├─→ R2 Экстрактор (извлечение знаний)
...
Все роли R1-R21 описаны в DP.AGENT.001 §3.2
Все агенты реализованы в DS-ai-systems (монорепо: 7 систем)
```
...
- **Живые примеры Pack:** [PACK-digital-platform](https://github.com/TserenTserenov/PACK-digital-platform) (40+ сущностей), [PACK-MIM](https://github.com/TserenTserenov/PACK-MIM) (22 сущности)
...
Каждый тир — конфигурация 5 измерений: знания, данные, ИИ-роль, действия, рабочее пространство. **T4 (IWE)** — это полноценная интеллектуальная рабочая среда, где человек работает совместно с ИИ-агентами через Claude Code.
...
**Где изучить:**
- [DP.ARCH.002 — тиры обслуживания](https://github.com/TserenTserenov/PACK-digital-platform/blob/main/pack/digital-platform/02-domain-entities/DP.ARCH.002-service-tiers.md) в PACK-digital-platform
- [DP.AISYS.014 — паспорт бота](https://github.com/TserenTserenov/PACK-digital-platform/blob/main/pack/digital-platform/02-domain-entities/DP.AISYS.014-aist-bot.md) (18 сценариев)
- [DP.AGENT.001 § 3 — реестр ИИ-систем и ролей](https://github.com/TserenTserenov/PACK-digital-platform/blob/main/pack/digital-platform/02-domain-entities/DP.AGENT.001-ai-agents.md)

---

## 11. Онтология

### 11.1. Что такое онтология в IWE
...
## 12. Архитектура обновлений IWE

### 12.1. Два потока обновлений

```
Авторская сторона (ежедневно):
