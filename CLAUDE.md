...
Контекст РП (фазы, handoff, ArchGate, бюджеты, решения) живёт в `{{GOVERNANCE_REPO}}/inbox/WP-NNN-*.md` для активных и `{{GOVERNANCE_REPO}}/archive/wp-contexts/` для закрытых. В реестре — только имя артефакта.
