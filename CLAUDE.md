...
---
## 9. Личное (L3 — только ваш IWE)

> **Это ваш персональный слой (L3).** Сюда — правила и различения, специфичные именно
> для вашей установки: имена ваших репозиториев, личные блокирующие правила, ваши доменные
> различения. Платформа этот раздел не перезаписывает: `update.sh` делает 3-way merge и
> сохраняет ваши изменения (если вы разошлись с базовой версией `.claude.md.base`).
>
> Кастомизацию протоколов/скиллов размещайте в `extensions/*.md` + `params.yaml`
> (см. Extensions Gate ниже), а НЕ прямой правкой платформенных файлов §1-§8.
...
**Для мейнтейнеров форка (`params.yaml → author_mode: true`):** прямое редактирование L1 файлов разрешено; доставка в шаблон — через `scripts/template-sync.sh` + promote-скрипты. Подробнее — в `STAGING.md` и `CONTRIBUTING.md`.

### Ваши блокирующие правила

<!-- Личные правила, которые всегда применяются в вашем IWE. Пример:
- **Без Obsidian:** просмотр заметок через редактор вместо Obsidian.
-->

### Ваши различения

<!-- Доменные различения, специфичные для вас. Можно держать также в
     .claude/rules/distinctions.md в зоне AUTHOR-ONLY — она не затирается update.sh. -->

### Именование ваших репозиториев

<!-- Например:
- `DS-<ваш-домен>` — ваш governance-хаб
- рабочая директория: `~/IWE/`
-->

### Read-only репозитории

<!-- Список репозиториев, которые агент не должен изменять. -->
