...
|---|---|---|---|
| Число файлов памяти | ≤11 | `CLAUDE.md §4` | **Совет** — счётчика нет ни в одном скрипте; создание счётчика/конфига под это число — отдельное архитектурное решение (ArchGate), не техническая правка issue #736 |
| HOT-слой суммарно | ≤150 строк | `verify-context-budget.sh` (эта спецификация §2) | **Исполняется** — блокирует коммит hot-файлов. Канон для всех мест ниже, которые измеряют тот же файл |
| `.claude/rules/distinctions.md` (платформенный hot-слой) | ≤150 строк (канон выше) | `verify-context-budget.sh`, `.claude/skills/week-close/SKILL.md` | **Исполняется** + drift-флаг Week Close — оба синхронизированы на 150 (issue #736; было 80 в week-close, расходилось с исполняемым порогом) |
...
| Collapsible-разметка файла-артефакта | >80 строк | `.claude/rules/formatting.md` | **Совет** — форматирование (когда сворачивать секции в markdown), НЕ бюджет памяти distinctions.md; разные измеряемые, число совпадало с прежним (ошибочным) порогом distinctions.md только по случайности |
| MEMORY.md (индекс) | >200 строк | `.claude/skills/week-close/SKILL.md:192` | **Совет** — агент проверяет глазами на Week Close; независимый критерий от размера в КБ ниже (строки ≠ байты/длина строки) — оставлено как есть, не сводится в одно число |
...
**Известные несведённые расхождения (не решается этой таблицей):** порог "≤11 файлов" не имеет технической реализации — оставлен как совет, решение о счётчике/новом числе за пилотом; порог индекса на Week Close (200 строк, ручной) и в `check-index-health.py` (КБ+длина строки/ячейки, автоматический) — два разных измеряемых одного файла (строки vs байты/длина), намеренно оставлены раздельными, не одно число.
