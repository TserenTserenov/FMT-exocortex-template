...
- **Knowledge stays in your head.** You completed a course, read a book, solved a problem — but a month later you cannot reconstruct the thinking.
...
- **No system.** Plans in notes, tasks in your head, Knowledge in chat logs. Everything is scattered.
- **Time disappears.** No clarity on what you worked on, what you accomplished, where you are going.
...
> **Key principle: exoskeleton, not prosthetic.** IWE amplifies your thinking — it does not replace it. After every Session you become more competent, not just better informed. See: [principles-vs-skills.md](docs/principles-vs-skills.md).
...
Every professional drowns in information: 12+ tools (Notion, Google Docs, Slack, ChatGPT, courses...), Knowledge spread thin, nothing connected. AI answers questions but does not know *your* context — starting from zero every time.
...
|----------|---------------|-----------|
| **Product development** | Claude knows the Architecture, technical debt, and Roadmap. Every Session continues — it does not start over | [SC.013](docs/use-cases/USE-CASES.md), [SC.015](docs/use-cases/USE-CASES.md) |
| **Documentation management** | Knowledge is captured in Pack during work. No need to "write docs later" — they are written as you work | [SC.004](docs/use-cases/USE-CASES.md), [SC.014](docs/use-cases/USE-CASES.md) |
...
| **Review and Refactoring** | ArchGate evaluates decisions across 7 characteristics. Not "seems fine" — a structured Assessment | [SC.015](docs/use-cases/USE-CASES.md) |
...
| **Strategizing** | A weekly Session: Review of the previous week, planning the next, alignment with goals. The Strategist prepares a draft — you make the decisions | [SC.011](docs/use-cases/USE-CASES.md) |
...
- In the morning — the Strategist has built a plan: a Telegram notification and a DayPlan file in the Repository.
- You open VS Code → `claude` → Claude knows what is in the plan and suggests starting with the Priority item.
...
A: For full setup (Claude Code) — Claude Pro ($20/month) is recommended. If needed, you can upgrade to Claude Max (~$100/month) for unlimited operation. For minimal setup (`setup.sh --core`) — works with any AI CLI. See: [SETUP-GUIDE.md](docs/SETUP-GUIDE.md).
...
Minimal setup (`setup.sh --core`) works without binding to a specific agent.

**Q: Does it work on Linux/Windows?**
A: Yes. The core works on any OS. Strategist automation: macOS — launchd, Linux — systemd (user units), cloud option (OS-independent) — GitHub Actions. Windows: `setup.sh` and the core run through Git Bash (installed with Git for Windows) — WSL is not required; WSL remains a fallback for those who prefer a full Linux layer. Not tested live on a real Windows machine (no Windows runner in CI) — details and honest caveats: [SETUP-GUIDE.md](docs/SETUP-GUIDE.md) § Windows.
...
**Q: Is my data secure?**
A: Three protection zones: local, GitHub (private repos), Platform (per-user isolation). See: [DATA-POLICY.md](docs/DATA-POLICY.md).
...
A: No. The template is a ready-made Configuration. Setup via setup.sh. Operation via Claude Code in natural language.
...
A: All setup commands must be run in the same terminal. Opening a new terminal starts from `~`. Delete the folder from `~` and repeat starting with `cd ~/IWE`. See: [SETUP-GUIDE.md](docs/SETUP-GUIDE.md).
...
The **IWE Community** is a closed chat for Practitioners working within the same system: ORZ, Pack, Exocortex. A place to discuss not "how to write better prompts" — but how to build intellectual work seriously.
...
Deep Practice, Work Product reviews, direct support. Access is through the **"IWE for Practitioners"** seminar (5000₽) via the [@aist_me_bot](https://t.me/aist_me_bot) bot.
...
**IWE team developers (T4+ level):** single entry point — [Getting Started for Developers](docs/developer/). In 10 minutes you will understand the development Pipeline (6 stations, dual output) and complete your first task.
