...
# An explicit environment value wins; otherwise retain the template default and
# legacy auto-detection for existing workspaces.
GOVERNANCE_REPO="${IWE_GOVERNANCE_REPO:-}"
if [ -z "$GOVERNANCE_REPO" ] && [ -d "$WORKSPACE_DIR/${IWE_GOVERNANCE_REPO:-DS-strategy}" ]; then
