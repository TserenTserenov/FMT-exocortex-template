...
- [behavior] WP-575: тип результата определяется до имени нового РП; системы и информационные объекты проходят соответствующие гейты. Уточнение после общего keyword обрабатывается по смыслу; при смене результата по решению пилота сохраняется история классификации.
...
- `8aea80d` feat(scripts): поставлены инструменты writer-barrier для канонического репозитория (WP-530 Ф5.2) (#791)

### Changed

- `ad51e34` docs(setup-guide): исправлено утверждение о приватности Aisystant MCP, добавлен сценарий индексации личного репозитория (#814)
- `2596226` chore(manifest): синхронизирована контрольная сумма `strategist.sh` после фикса `ds-publish.sh`
- `17e6d92` docs(artifactor): `SKILL.md` приведён в соответствие с текущим поведением (WP-481 Ф7 + WP-575 Ф2) (#794)
- `e5f4356` docs(adr-004): добавлена сноска-прецедент ADR-006 — второй репозиторий семьи MC (#780)
- `82c59b4` docs(manifest-coverage): задокументировано, почему `validate_manifest_coverage.sh` не подключён к CI (#775)
- `0d41a84` docs: добавлена инструкция подключения ChatGPT Work и Codex MCP (#751)

### Fixed

- `294cf6d` fix: разбор issue-шаблона — 11 фиксов (РП-7 Ф150) (#839)
- `5e75aca` fix(update): валидация канала обновления и заметное предупреждение без Python (#838)
- `ed12e08` fix(session-guard): запись `governance_worktree` при открытии (WP-484, точечный патч) (#829)
- `9add3ea` fix: 3 фикса из issue-триажа — путь scaffold (#826), обход dry-run-gate через nice/basename (#825), утечка trap в stop-gate (#818) (#828)
- `21323ec` fix(hooks): бесконечный цикл stop-gate + обход dry-run через uv/uvx (#824)
- `d24cb72` fix(extractor): конвейер inbox-check не видел захваты из браузерного источника (#822)
- `677da9a` fix: закрыт обход whitelist `.iwe-paths` в `dry-run-gate.sh` через `HOME`/`IWE_PATHS_FILE` (#815)
- `afdcf7d` fix: whitelist `.iwe-paths` в `dry-run-gate.sh` (на основе nonce) + слияние `mcp.json` Т3/Т4 (#813)
- `84fb3bb` fix: 5 пробелов, найденных независимой проверкой результата #811 (#812)
- `c281adf` fix: 5 фиксов из issue-триажа — #787, #786, #781, #804 (#811)
- `23530c2` fix(wp-170): следование контракту переносимого Python и опубликованным снапшот-тестам
- `baefe1f` fix(wp-170): сохранение решений экстрактора + требование наблюдаемой доставки
- `cee9d79` fix: сохранение содержимого дня + восстановление прямых записей закрытия в журнале (#802)
- `1a41e49` fix(agent-inbox): синхронизация зеркала диспетчера (WP-564 Ф3) (#798)
- `4b38def` fix(wp484): удалена недостижимая legacy-ветка Kimi
- `1e11c5c` fix(wp484): устойчивый к падениям адаптер напарника Kimi
- `8ec5c04` fix(strategist): публикация через `ds-publish.sh` вместо сырого pull/push
- `5ca5bae` fix(wp484): устойчивый к падениям жизненный цикл напарника Kimi
- `0ccd7f9` fix(wp484): сохранена совместимость извлечённого закрытия
- `b355e80` fix(wp484): усилен жизненный цикл сессии шаблона
- `8ca2a26` fix(ar293): headless-запуски Экстрактора/Стратега переключаемы на другую программу (#792)
- `5dfeb27` fix(setup): fail-closed при неоднозначных локальных кандидатах governance-репозитория (#790)
- `c0f3458` fix(artifactor): устранены 3 нарушения правила именования в `KEYWORD_MAP` (#789)
- `7ea708a` fix(sync): разбор связей РП без ложных совпадений (#788)
- `f39f1ff` fix(wp526): генератор `install-iwe-paths.sh` не прописывал `IWE_SESSIONS_ROOT` (#669)
- `c3662b3` fix(setup,memory): доработки по итогам пост-релизного аудита — issue #748 (#779)
- `34b3495` fix(hooks,scripts): закрыты issue из триажа — #757, #758, #759, #772, #773, #736 (#777)
- `c50c7f0` fix(day-open): конвейер использовал жёстко зашитый `$IWE/scripts` вместо `$IWE_SCRIPTS` (#776)
- `0395e72` fix(update): побочные эффекты на чужую копию, миграция каталога, тихий сбой хеша (#774)
- `00bfded` fix(peer-adapter-filter): паттерны каталогов с завершающим слэшем в `.agentigore` никогда не срабатывали (#771)
- `ba3fa3a` fix(mandatory-gate): резолвер `find-python3.sh` не находил ни один реальный путь установки (#769)
- `cc470ef` fix(session-guard): YAML-экранирование поля `task` при записи семафора (WP-539) (#770)
- `3e2db35` fix: обязательный `verification_class` при создании РП, встроено напоминание про декомпозицию в карточку (#766)
- `e69fca5` fix(hooks,translate-sync): переносимый timeout, путь реестра, буквенные подсекции (#762)
- `af4ba23` fix(residency-gate): различает свежую установку от пропавшего состояния согласия (issue #521B) (#763)
- `2bd6cfd` fix(translate): добавлено имя в `frontmatter_translate_keys` (issue #749) (#750)
