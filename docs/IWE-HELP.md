...
| **Описания** | Знания, загружаемые в системы | FPF/ZP, Pack-сущности, промпты ролей, содержимое экзокортекса |
| **Роли** | Функция, не исполнитель | Стратег (R1) ← Claude, Экстрактор (R2), Синхронизатор (R8), Пользователь ← Человек |
| **Методы** | Процедуры «как делать» | Протокол ОРЗ, Capture-to-Pack, KE, Note-Review |
...
**Как создать:** выбор домена (не темы) → имя Pack (существительное) → структура → дорожная карта наполнения.
