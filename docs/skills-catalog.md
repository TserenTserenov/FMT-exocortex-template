...
| `/apply-captures` | Processes extraction reports with status pending-review — resolves R15 (accept/reject/defer) by a LIVE PILOT, records… |
...
| `/audit-installation` | Audits the user's IWE installation. Runs scripts/iwe-audit.sh + MCP healthcheck + ritual smoke-test… |
| `/author-mode` | Instructions for the IWE Template author: staging channel (test run → FMT), author rules L3, Extensions Gate. Load… |
| `/bottleneck-pick` | Constraints Analyst (DP.ROLE.054): finds the primary constraint (bottleneck) of a specific Pipeline via TOC… |
...
| `/diagnose` | Diagnoses the Mastery stage (Diagnostician R28, FORM.089 §6.1 v5.0) directly in VS Code / claude.ai. Up to 6 questions… |
| `/discovery-session` | Dissatisfaction unpacking conversation (discovery strategizing). Socratic breakdown of raw reflection… |
...
| `/iwe-bug-report` | Report an IWE Platform error or issue. Creates a GitHub issue in FMT-exocortex-template. |
...
| `/local-llm` | Local LLM stack on Mac (Apple Silicon, MLX) for privacy and fallback mode. NL entry point to install/run/… |
| `/month-close` | Month Close protocol. Stage 7 of the OWC v9 cascade (PD.METHOD.008). Runs on the first Monday of the month… |
...
| `/peer-conversation` | Multi-turn dialogue between the writer (Claude) and Peer partner (Kimi) on a pilot task (DP.SC.154). Runs the turn loop, detects… |
| `/personal-guide-render` | Thin client — assembles the pilot's personal guide via the local gateway (mcp__iwe-local-gateway__render… |
...
| `/vdv` | OWC Skill — generator and auditor of a staged process description based on the 6 Opening·Action·Closing principles. Use for… |
...
| `/week-close` | Week Close protocol. Retrospective for 7 days + carry-over to the new week + platform steps (Backup… |
