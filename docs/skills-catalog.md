...
| `/bottleneck-pick` | Constraint analyst (DP.ROLE.054): identifies the primary constraint (bottleneck) of a specific Pipeline via TOC… |
...
| `/diagnose` | Mastery stage diagnosis (Diagnostician R28, FORM.089 §6.1 v5.0) directly in VS Code / claude.ai. Up to 6 questions,… |
...
| `/peer-conversation` | Multi-turn dialogue between a writer (Claude) and a Peer partner (Kimi by default, Codex as second vendor) on a pilot task… |
...
| `/vdv` | OWC Skill — generator and auditor of a staged process description based on the 6 Input·Action·Output principles. Use for… |
...
| `/week-close` | Week Close Protocol. 7-day Retrospective + carry-over to new week + Platform steps (Backup,… |
| `/week-close-pilot` | Run the pilot-facing Week Close protocol with progress Review and carry-over. |
| `/wp-new` | Creates a new Work Product (WP) with atomic write to 4 local locations (+ conditional post-step to external… |
