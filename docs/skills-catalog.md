...
| `/discovery-session` | Conversation for unpacking dissatisfactions (discovery strategizing). Socratic analysis of raw reflection and… |
...
| `/lesson-close` | Close a lesson opened with /lesson. Finalizes workbook/YYYY-MM-DD.md (frontmatter status, metadata… |
...
| `/vdv` | VDV Skill — generator and auditor for staged process descriptions based on 6 Input·Action·Output principles. Use for… |
| `/verify` | Verification of an Artifact against a reference from the Pack. Loads Role VR.R.001 (Verifier) with context isolation — checks… |
