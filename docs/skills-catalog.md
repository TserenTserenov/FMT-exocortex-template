...
| `/apply-captures` | Processes extraction reports with status pending-review — LIVE PILOT resolves R15 (accept/reject/defer) and records… |
...
| `/author-mode` | Instructions for the IWE Template author: staging channel (testing → FMT), L3 authoring rules, Extensions Gate. Load… |
...
| `/discovery-session` | Conversation-unpacking of dissatisfactions (discovery-strategizing). Socratic breakdown of raw reflection and… |
...
| `/iwe-bug-report` | Report a bug or issue on the IWE Platform. Creates a GitHub issue in FMT-exocortex-template. |
...
| `/peer-conversation` | Multi-turn writer dialogue (Claude) with one or more Peer partners (any combination of kimi/codex/hermes/c… |
...
| `/vdv` | OWC Skill — generator and auditor for a staged process description based on 6 Input·Action·Output principles. Use for… |
| `/verify` | Verification of an Artifact against a Pack reference. Loads Role VR.R.001 (Verifier) with context isolation — checks… |
...
| `/wp-new` | Creates a new Work Product with atomic write to 4 local locations (+ conditional post-step to external… |
