...
| `check-orphan-hooks.sh` | `scripts/check-orphan-hooks.sh` | verifies that every hook in .claude/hooks/ is actually invoked |
...
| `check-wp-transfer-completeness.sh` | `scripts/check-wp-transfer-completeness.sh` | pre-archival check for inbox/WP-N/: |
...
| `pack-ci-install.sh` | `scripts/pack-ci-install.sh` | Installs CI guard (ID collision detector) into all Pack repos under ~/IWE/ |
...
| `pre-commit-secret-scan.sh` | `scripts/pre-commit-secret-scan.sh` | Pre-commit hook: blocks accidental commit of secrets. |
...
| `skills-pull.sh` | `scripts/skills-pull.sh` | sync L1 skills from FMT into a personal IWE |
