...
> Auto-generated by `scripts/generate-catalogs.py` · 2026-08-04 · DO NOT edit manually.
...
| `check-orphan-hooks.sh` | `scripts/check-orphan-hooks.sh` | verifies every hook in .claude/hooks/ is actually invoked |
...
| `check-seed-drift.sh` | `scripts/check-seed-drift.sh` | verifies that seed/strategy/scripts/ snapshots have not diverged from scripts/ |
| `check-setup-update-parity.sh` | `scripts/check-setup-update-parity.sh` | static analysis of paired scripts |
| `check-wp-transfer-completeness.sh` | `scripts/check-wp-transfer-completeness.sh` | pre-archive check for inbox/WP-N/: |
...
| `day-open-pipeline.sh` | `scripts/day-open-pipeline.sh` | full Day Open Pipeline orchestrator (WP-356 DOE3) |
...
| `hook-promote.sh` | `scripts/hook-promote.sh` | promote a personal hook into the IWE platform template |
...
| `memory-drift-scan.py` | `.claude/scripts/memory-drift-scan.py` | Status drift detector between MEMORY.md and WP-context. |
| `memory-health.sh` | `scripts/memory-health.sh` | memory/ health metrics (WP-217 Phase 10.2) |
| `memory-migrate.sh` | `scripts/memory-migrate.sh` | add missing frontmatter fields (WP-217 Phase 10.2/10.4) |
...
| `migrate-to-runtime-target.sh` | `scripts/migrate-to-runtime-target.sh` | migrate from dirty FMT (≤0.28.x) to Generated runtime (≥0.29.0). |
| `pack-ci-install.sh` | `scripts/pack-ci-install.sh` | Installs the CI guard (ID collision detector) into all Pack repositories under ~/IWE/ |
| `peer-session-finalize.sh` | `scripts/peer-session-finalize.sh` | finalize / interrupt for peer sessions (DP.SC.154) |
| `pending-phases-sweep.sh` | `scripts/pending-phases-sweep.sh` | sweeps active WP-context files and outputs pending phases |
...
| `script-promote.sh` | `scripts/script-promote.sh` | promote a personal script (or all shared scripts) into the IWE platform template |
...
| `settings-promote.sh` | `scripts/settings-promote.sh` | register a hook in the template's .claude/settings.json |
| `setup-extractor-feeders.sh` | `scripts/setup-extractor-feeders.sh` | Onboarding script for activating the feeder system |
| `skill-promote.sh` | `scripts/skill-promote.sh` | promote a skill into the IWE platform template (v2.1) |
...
| `style-check-post-run.sh` | `scripts/style-check-post-run.sh` | check Kimi response style after a peer session (WP-388 Phase 9) |
| `style-promote.sh` | `scripts/style-promote.sh` | promote a style snapshot file into the IWE platform template |
...
| `sync-version-badge.sh` | `scripts/sync-version-badge.sh` | sync version badge in README.md with CHANGELOG.md |
...
| `week-draft-append.sh` | `scripts/week-draft-append.sh` | update the current day's metrics in the weekly post draft. |
...
| `wp499-agent-coordination-test.sh` | `scripts/wp499-agent-coordination-test.sh` | Reproducible N-agent coordination test for IWE (WP-499 Phase 7/11, extended Phase 17 items 3–4, |
