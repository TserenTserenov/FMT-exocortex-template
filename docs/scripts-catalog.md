...
| `add-secret.sh` | `scripts/add-secret.sh` | Single method to save any API key/token to ~/.secrets/ |
...
| `check-orphan-hooks.sh` | `scripts/check-orphan-hooks.sh` | Verifies every hook in .claude/hooks/ is actually called |
...
| `day-open-checks-runner.sh` | `scripts/day-open-checks-runner.sh` | Parser and executor for bash blocks in extensions/day-open.checks.md |
...
| `generate-rules-registry.py` | `.claude/scripts/generate-rules-registry.py` | generate-rules-registry.py — builds rules-registry.yaml from PACK-agent-rules/rules/AR.NNN.md. |
...
| `iwe-backup-check.sh` | `scripts/iwe-backup-check.sh` | Health check for the IWE Backup system |
...
| `memory-health.sh` | `scripts/memory-health.sh` | Health metrics for memory/ (WP-217 Phase 10.2) |
...
| `pack-ci-install.sh` | `scripts/pack-ci-install.sh` | Installs the CI guard (ID collision detector) into all Pack repositories in ~/IWE/ |
...
| `staging-audit.sh` | `scripts/staging-audit.sh` | Detector for B12e Decay drift in STAGING.md |
