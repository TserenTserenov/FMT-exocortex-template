...
- **Персональное руководство** — опциональный маршрут развития, собранный из твоих целей и контекста
...
| **2а** | Персональное руководство | 10-20 мин | можно позже |
...
> **⚠ Сетевые ограничения (Россия и похожие регионы).** Часть загрузок ниже (GitHub, npm, Homebrew) может быть недоступна напрямую. Стандартные обходы: VPN, системный прокси, либо `torify <команда>` для отдельного вызова — например `torify curl ...` или `torify git clone ...` (для `npm`/`brew` `torify` не всегда достаточно, там чаще нужна собственная настройка прокси/реестра пакетного менеджера). Если конкретный шаг ниже не проходит — прежде чем разбираться в ошибке, проверь, не в этом ли дело.
...
**Если не получается войти через браузер** (`Login with a web browser`) — на шаге выбора способа возьми `Paste an authentication token` вместо него. Эта опция не открывает браузер сама, `gh` ждёт готовый токен:

1. Открой `https://github.com/settings/tokens` в браузере.
2. Создай токен: **classic** (`Tokens (classic)` — отдельная вкладка/ссылка, страница по умолчанию открывается на fine-grained), с правами как минимум `repo` и `workflow` (нужен, чтобы форкнутый шаблон мог пушить `.github/workflows`). Classic проще для этого сценария — понятно, какие права дать; не ограничивай токен конкретными репозиториями (`setup.sh` создаёт новые).
3. Скопируй токен и вставь его в терминал по запросу `gh auth login`.
4. Проверь тем же `gh auth status`, что и выше.
...
Если используешь Obsidian, открывай как vault только отдельный
governance-репозиторий `DS-strategy`. Корень IWE (`~/IWE`) как Obsidian vault
не поддерживается: большие технические Markdown-файлы, например
`FPF/FPF-Spec.md`, могут повесить индексатор и оставить белый экран. Весь
workspace открывай через VS Code; технические репозитории не подключай в
governance-vault через символические ссылки.
...
|-------|----------------|
| `update.sh` | берёт обновления из зашитого адреса оригинала (`UPSTREAM-CONST`), а не из твоего форка или копии, поэтому смена форка на копию источник обновлений не меняет. Одно исключение: в канале release проверка отката делает `git fetch origin <коммит релиза>` в твоём клоне; если этого коммита у копии нет, автоприменение с `--yes` останавливается, интерактивный запуск работает |
| `scripts/fmt-critical-alert.sh` | у копии нет «родителя», поэтому скрипт проверит **свой** список issue, а он пуст. Ответ «0 (✅ clean)» в этом случае значит «проверять нечего», а не «проблем нет» (если трекер выключен, скрипт завершается с «cannot check»). Задай `IWE_FMT_REPO=TserenTserenov/FMT-exocortex-template` (переменная окружения или строка в `.exocortex.env`), чтобы он смотрел issue оригинала |
...
> Ниже — путь для Claude Code (через `claude.ai`). Для других агентов (Hermes и т.п.) см. `AGENT-VENDOR-SETUP.md`, Шаг 6.
...
### 1.3c Подключи свои репозитории к личному поиску (опционально)

Шаги 1.3b дают Claude доступ к базе знаний платформы. Твои собственные репозитории (заметки, Pack, рабочие проекты) в этот поиск сами по себе не входят — их нужно подключить отдельно.

**Как подключить:** напиши агенту «подключи мой GitHub». Он вызовет инструмент `github_connect` и даст ссылку на установку GitHub App `aisystant-knowledge`. По ссылке ты сам на странице GitHub выбираешь, какие репозитории открыть, и нажимаешь Install — отдельно ничего донастраивать не нужно. Индексируется только то, что ты явно отметил.

> **Про приватность:** содержимое выбранных репозиториев (включая приватные) покидает твою машину и GitHub и хранится на сервере платформы в виде поискового индекса — это нужно для самой возможности искать по смыслу, а не просто по словам. Не подключай репозиторий, если не готов к этому.

После установки индексация идёт сама: первичная — обычно быстро, для больших репозиториев может занять дольше; дальше каждый push переиндексируется автоматически, без участия локальной машины.
...
### Персональное руководство (до 60 мин, экспериментально и опционально)

Этот шаг не нужен для работы IWE; безопасный выбор — пропустить его, пока не
нужно именно публичное руководство. Команда `/personal-guide-start` не задаёт
уточняющих вопросов: её первое действие — создать или переиспользовать внешний
**публичный** GitHub-репозиторий `DS-personal-guide` (`private: false`). Запускай её
только если намеренно согласен на этот внешний эффект.

До запуска нужны активная подписка «Инженерия интеллекта», подключённый GitHub
в Aisystant MCP и доступные операции `create_repository` и `github_status`.
Наполнение шести файлов выполняет отдельная команда `/personal-guide-render`;
ей дополнительно нужны Память.Derived и операция `personal_write`. Если
серверный renderer или эти операции недоступны, bootstrap не завершится: в
GitHub уже может остаться созданный публичный репозиторий без готовых шести
файлов. Это не следует считать готовым персональным руководством.

Если сама команда не видна, проверь наличие
`.claude/skills/personal-guide-start/SKILL.md` и перезапусти Claude Code. Если
команда видна, но серверные операции недоступны, не запускай её повторно вслепую
— сначала восстанови подключение GitHub/Aisystant MCP и renderer.
...
| **Aisystant MCP** (поиск по базе знаний) | Сервер платформы (mcp.aisystant.com), OpenRouter (эмбеддинг запроса) | Текст поискового запроса. Содержимое твоих файлов не отправляется, пока не подключишь свои репозитории (см. §1.3c) |
| **Aisystant MCP** (если подключил свои репозитории, §1.3c) | Сервер платформы (mcp.aisystant.com), OpenRouter (эмбеддинг содержимого) | Содержимое подключённых репозиториев, включая приватные — уходит на сервер для построения личного поиска |
