...
routing:
  executor: script
  deterministic: true
  script_path: ".agents/skills/extend/show-catalog.sh"
---
...
| `auto_verify_code` | Quick Close | `true` | Автоверификация кода лёгкий субагент |
| `verify_quick_close` | Quick Close | `true` | Верификация чеклиста лёгкий субагент |
...
#### Свои навыки (.agents/skills/)

Создать `.agents/skills/<name>/SKILL.md` — skill будет доступен как `/<name>`.
