...
- **Фаза B — исполнитель.** ОДИН general-purpose субагент (sonnet). В промпт: (а) дайджест целиком, (б) сегодняшняя дата, (в) инструкция: «Прочитай `.agents/skills/day-close/SKILL.md`, выполни шаги 1-7 через update_plan, применяя карту замен ниже. Шаг 0 пропусти — родитель уже выполнил. Форматирование таблиц — `.claude/rules/formatting.md`. Git: стейджить только конкретные файлы, НЕ коммитить. Верни: черновик итогов (шаг 7), список правленных файлов, блокеры». Приложить карту замен.
...
**update_plan:** родитель ведёт фазы 0б/B/C/D/E как задачи; каждый субагент ведёт свои SKILL-шаги собственным update_plan — блокирующее правило пошаговости соблюдено.
