...
Month Close = протокол. Исполнять ТОЛЬКО пошагово через update_plan.
...
- **T24 Аудит протоколов на обходимые шаги.** Пройти `memory/protocol-*.md` и `.agents/skills/*/SKILL.md` — выявить шаги, которые агент регулярно пропускает (по `behaviour-report.sh`) или человек обходит молча. Решение: удалить / переформулировать / оставить как есть.
