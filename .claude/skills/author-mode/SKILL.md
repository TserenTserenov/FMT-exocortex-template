...
3. **Режим (ADR-IWE-014 R9, WP-552 Ф5):** спросить — обязательно для всех инсталляций шаблона, или отключаемое расширение маркетплейса? Ось ортогональна R8 и решается отдельно от готовности из шага 1. Отключаемое расширение → см. «Тип `extension`» ниже; обязательное правило шаблона → шаг 4 напрямую.
...
**Тип `extension` (отключаемое расширение маркетплейса):** `promote-common.sh` пишет запись с `type=extension` не в `promotion-status.yaml`, а в `marketplace-manifest.yaml` — поля `owner`, `install_point`, `permissions`, `toggleable`, `conflict_rule`, `staging_source` (ссылка на строку `STAGING.md` для аудируемости — какое правило и когда прошло тест R8). Использует существующий тип артефакта (Pack, skill, `AR.NNN`, роль), не создаёт нового.
