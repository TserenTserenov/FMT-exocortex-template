...
### 9. Запись итогов
**9a.** Дописать «Итоги дня» в DayPlan (шаблон: `memory/templates-dayplan.md`).
Тем же редактированием заменить в frontmatter DayPlan `status: active` на `status: closed` (машинный признак закрытия дня).
Валидация: «Завтра начать с» непустое + каждый pending РП с конкретным next action. Postcondition: bash-grep по паттерну `Итоги дня|Day summary` (оба языка — issue #234: при `language: english` заголовок DayPlan «Day summary», русский grep всегда FAIL) → `9a OK/FAIL`.
...
- [ ] Итоги дня записаны в DayPlan, `status: closed` проставлен **(postcondition 9a: grep подтверждён)**
