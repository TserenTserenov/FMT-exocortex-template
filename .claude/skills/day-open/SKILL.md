...
`cd "$IWE_TEMPLATE" && bash update.sh --check` + проверка Base-репо (FPF, ZP) на отставание от origin. Обновления → «Требует внимания». Scout report не проревьюен → «Требует внимания».
