...
```
Если есть новые коммиты → добавить в «Требует внимания»: «[repo] обновлён upstream → `cd {{WORKSPACE_DIR}}/[repo] && git pull --rebase`». После pull FPF/SPF → reindex: `bash {{WORKSPACE_DIR}}/DS-MCP/knowledge-mcp/scripts/selective-reindex.sh FPF` (или SPF). Reindex обновит данные, доступные через Gateway (iwe-knowledge).
